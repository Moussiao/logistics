...
from apps.orders.models import Customer, CustomerAddress, Order, Partner, Product
from apps.orders.services.exceptions import (
...
if TYPE_CHECKING:
    ...
    from apps.orders.api.schemas import OrderInput
...
class CreateOrder:
    ...
    def _create_customer_address(
        ...
        region, _x = Region.objects.get_or_create(
            country=country, name=address_schema.region_name
        )
        city, _x = City.objects.get_or_create(
            region=region, name=address_schema.city_name
        )
