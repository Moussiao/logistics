...
from backend.apps.security.api.schemas import TokenRequest
from backend.apps.security.jwt import decode_user_access_token
from backend.apps.security.services import (
    CreateAccessToken,
...
if TYPE_CHECKING:
    ...
    from backend.apps.tg_bots.tests.fixtures import TgUserFactory
...
def mock_init_data_validate(mocker: "MockerFixture") -> "MockType":
    return mocker.patch(
        "backend.apps.tg_bots.mini_app.InitDataValidator.validate", return_value=None
    )


@pytest.mark.django_db
def test_create_access_token(
    user: "User", tg_user_factory: "TgUserFactory", mock_init_data_validate: "MockType"
) -> None:
...
def test_user_not_found_error(
    tg_user_factory: "TgUserFactory", mock_init_data_validate: "MockType"
) -> None:
