...
from backend.apps.delivery.tasks import notify_of_buyout_orders, notify_of_new_orders
...
def mock_try_send_reports(mocker: MockerFixture) -> MockType:
    return mocker.patch(
        "backend.apps.delivery.services.reports.TrySendReports.send_to_active_partners"
    )
